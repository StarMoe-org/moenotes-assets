...
are skipped, not downloaded. USM alpha video (`@ALP`) and empty sprite atlases are
detected after download and also skipped. `skipped` on the task counts them; export
task `results` list only successful and failed items, so a full-catalog task stays
small. Completed includes successful, skipped and failed items. A succeeded task can
contain skips; compare completed, skipped and results when checking export coverage. Missing keys
...
`csharp-usm-mp4-vp9copy-h264-aac-v2`, so video codec changes leave other exports
reusable. Video files remain `video/mp4`; VP9 sources are copied, others are H.264.
Version 2 decodes USM ADX audio with VGAudio instead of FFmpeg.
