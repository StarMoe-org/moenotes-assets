...
internal static class CriFixture
{
    ...
    public static byte[] Acb(byte[] hca, bool streaming = false, bool blocks = false)
    {
        ...
        // A block sequence (looping BGM) reaches the track through a block instead of its own track list.
        return Utf(("AwbFile", awb), ("CueTable", Utf(("CueId", 100), ("ReferenceType", blocks ? 8 : 3), ("ReferenceIndex", 0))),
            ("CueNameTable", Utf(("CueIndex", 0), ("CueName", "synthetic-tone"))),
            blocks ? ("BlockSequenceTable", Utf(("NumTracks", 0), ("TrackIndex", Array.Empty<byte>()), ("NumBlocks", 1), ("BlockIndex", new byte[2]))) : ("SequenceTable", Utf(("NumTracks", 1), ("TrackIndex", new byte[2]))),
            ("BlockTable", Utf(("NumTracks", 1), ("TrackIndex", new byte[2]))),
    ...
    // plain: streams written without the CRI mask, as some encoders do; audioCodec false omits the @SFA audio_codec column.
    public static byte[] Usm(byte[] video, ulong key, int frames = 0, int rate = 25, int codec = 1, byte[]? adx = null, bool alpha = false, bool plain = false, bool audioCodec = true)
    {
        ...
        if (bytes.Length >= 0x240 && !plain)
        {
            ...
            for (int i = 0x140; i < bytes.Length; i++) { int lane = (i - 0x140) & 31; var original = bytes[i]; bytes[i] ^= state[lane]; state[lane] = (byte)(original ^ ~mask[lane]); }
        }
        ...
        if (adx != null)
        {
            ...
            var audio = (byte[])adx.Clone(); if (!plain) for (int i = 0x140; i < audio.Length; i++) audio[i] ^= audioMask[(i - 0x140) & 31];
            Chunk("@SFA", 1, audioCodec ? Utf(("audio_codec", 2)) : Utf(("ixsize", 0))); Chunk("@SFA", 0, audio); Chunk("@SFA", 2, "#CONTENTS END"u8.ToArray());
        }
